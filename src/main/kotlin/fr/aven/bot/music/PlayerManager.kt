...
class PlayerManager(config: Config) {
    ...
    init {
        ...
        else YoutubeIpRotatorSetup(NanoIpRoutePlanner(listOf(Ipv6Block(ipv6)), true)).forSource(
            youtubeAudioSourceManager).setup()
    ...
    fun loadAndPlayDeezerTrack(interaction: SlashCommandInteraction, track: Track) =
    ...
    fun loadAndPlay(interaction: SlashCommandInteraction, trackUrl: String) {
        ...
        playerManager.loadItemOrdered(musicManager, trackUrl, object : AudioLoadResultHandler {
            ...
            override fun playlistLoaded(playlist: AudioPlaylist) {
                if (playlist.isSearchResult) musicManager.searchResult(playlist, interaction)
                    ...
                    interaction.replyEmbeds(Embed {
                        author {
                            name = musicManager.language.getString(LangKey.keyBuilder(this,
                                "playlistLoaded",
                                "playlist.title"), "Playlist added")
                        }

                        field {
                            name = "❱ ${playlist.name} (${playlist.tracks.size} tracks)"
                            value = "❱ ${
                                musicManager.language.getString(LangKey.keyBuilder(this,
                                    "playlistLoaded",
                                    "playlist.firstTrack"), "First track")
            ...
            override fun noMatches() {
                interaction.reply(musicManager.language.getString(LangKey.keyBuilder(this,
                    "noMatches",
                    "music.notFound"), "Nothing found, please retry (link recommended).")).queue()
            }

            override fun loadFailed(exception: FriendlyException?) {
                interaction.reply(musicManager.language.getString(LangKey.keyBuilder(this,
                    "noMatches",
                    "music.couldntPlay"), "Could not play.")).queue()
            }
